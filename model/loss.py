...
def vat_loss(device, input_dict, custom_embedding, custom_LSTM, custom_classifier, X, logit_for_v, vat_epsilon,
             hyperpara_for_vat):
    embedded = custom_embedding(X)  # sent_len * bsz * embedding_dim
...
def kl_divergence(logit_for_v, new_logit):
    ...
    kl = torch.sum(
        (F.log_softmax(logit_for_v, dim=-1) - F.log_softmax(new_logit, dim=-1)) * F.softmax(logit_for_v, dim=-1), -1)
